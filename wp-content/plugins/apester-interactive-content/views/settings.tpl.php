<div class="wrap" id="apester-admin">
    ...
    // usersIframe URL
    define('USERS_IFRAME', 'https://static.apester.com/cookie/index.html');
    ?>
    ...
    <iframe src="<?php echo esc_url(USERS_IFRAME) ?>" id="users-iframe" height="1" width="1" style="position: absolute;"></iframe>    
    <script>
        (jQuery)(function($) {
            var usersIframe = document.getElementById('users-iframe');
            var usersIframeLoaded = false;
            $(usersIframe).on('load', function() {
                usersIframeLoaded = true;
            });
            ...
            apesterUtils.getChannelName = function(channelToken) {
                ...
                function handleGetChannelMessage(event) {
                    if ((<?php echo wp_json_encode(USERS_IFRAME) ?>).indexOf(event.origin) === -1) return
                    window.removeEventListener('message', handleGetChannelMessage)
                    var response = event.data;
                    deferred.resolve(typeof response.payload !== 'undefined' && response.payload !== null ? response.payload.name : channelToken);
                }
                
                var deferred = $.Deferred();     
                if (usersIframeLoaded) {
                    window.addEventListener('message', handleGetChannelMessage);
    
                    usersIframe.contentWindow.postMessage({
                        action: 'get-channel',
                        params: [channelToken]
                    }, <?php echo wp_json_encode(USERS_IFRAME) ?>);
                } else {
                    $(usersIframe).on('load', function() {
                        usersIframeLoaded = true;
                        window.addEventListener('message', handleGetChannelMessage);
            ...
            window.usersIframe = usersIframe;
        });
    </script>
        ...
        <form id="settingsTab" method="post" action="options.php">
            ...
            <script>
                (jQuery)(function($){
                    ...
                    var usersIframe = window.usersIframe;
                    ...
                    var attachChannelNames = function() {
                        ...
                        tokenLabels.each(function(){
                            var $this = $(this);
                            var token = $this.text();
                            apesterUtils.getChannelName(token).done(function(channelName) {
                    ...
                    $("#settingsTab").submit(function() {
                        ...
                        var sendEvents = function(addedTokens, removedTokens) {
                            ...
                            var updateSingleTokenToWP = function(channelToken, type) {
                                usersIframe.contentWindow.postMessage({
                                    action: 'update-channel',
                                    params: [channelToken, type]
                                }, <?php echo wp_json_encode(USERS_IFRAME) ?>);
                            };
                            ...
                            for (i=0; i < addedTokens.length; i++) {
                                ...
                                updateSingleTokenToWP(addedTokens[i], 'wordpress');
                            }

                            for (i=0; i < removedTokens.length; i++) {
                                sendEvent(eventNames.removed, removedTokens[i]);
                                updateSingleTokenToWP(removedTokens[i], 'jsEmbed');
                            }
                        };
                    ...
                    <div class="pull-left ape-controls">
                        ...
                        <script>
                            (jQuery)(function($){
                                ...
                                tokenLabels.each(function(){
                                    var $this = $(this);
                                    apesterUtils.getChannelName($this.text()).done(function(channelName) {
